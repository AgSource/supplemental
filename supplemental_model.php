 ...
 class Supplemental_model extends CI_Model{
	...
	* -----------------------------------------------------------------*/
     ...
     * -----------------------------------------------------------------*/
     function getDataSuppByBlock($block_id) {
         $ret = $this->db
             ->select('block_id, display_type_id, content_id, content_column_id, db_field_name, supp_id, a_href, a_rel, a_title, a_class')
             ->where('block_id', $block_id)
             //->order_by('p.list_order', 'ASC')
             ->get('users.[dbo].[v_block_supplemental_data]')
             ->result_array();
         return $ret;
     }
    ...
    * -----------------------------------------------------------------*/
     function getHeaderSuppByBlock($block_id) {
         $ret = $this->db
             ...
             ->where('block_id', $block_id)
             //->order_by('p.list_order', 'ASC')
             ->get('users.[dbo].[v_block_supplemental_header]')
             ->result_array();
         return $ret;
     }
    ...
    * -----------------------------------------------------------------*/
     function getControlSuppByForm($form_id) {
         $ret = $this->db
             ->select('block_id, form_id, db_field_name, supp_id, a_href, a_rel, a_title, a_class')
             ->where('form_id', $form_id)
             //->order_by('p.list_order', 'ASC')
             ->get('users.[dbo].[v_form_supplemental]')
             ->result_array();
         return $ret;
     }
 }